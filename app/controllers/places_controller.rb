class PlacesController < ApplicationController
  ...
  def show
    ...
    if user_signed_in?
      ...
      if @review.exists?
        @review = @review.first
        @reviews = Review.where(place_id: @place.id).where.not(user_id: current_user.id).order(created_at: :desc).paginate(page: params[:page], :per_page => 5)
      else
        @review = current_user.reviews.build
        @reviews = @place.reviews.order(created_at: :desc).paginate(page: params[:page], :per_page => 5)
      end
    else
      @reviews = @place.reviews.order(created_at: :desc).paginate(page: params[:page], :per_page => 5)
    end
